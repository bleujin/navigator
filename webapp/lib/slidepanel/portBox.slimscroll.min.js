...
(function (e) {
	...
	e.fn.showPortBox = function(t) {
		t.preventDefault();

		if (codeEditor) {
			var nodeId = e(this).attr('id'), nodePath = _stage.idToPath(nodeId);

			if(_stage.isUnsavedNode(nodeId)) {
				noty({
					layout : 'topCenter',
					text   : 'Unsaved node. <br/>Connect to somewhere first',
					type   : 'error',
					timeout: 2000
				});
				return;
			...
			$.ajax({
				url     : NC.REST.propertyLet + nodePath,
				dataType: 'json',
				type    : 'GET',
				success : function (data) {
					_stage.propertyPanel().setNodePath(nodePath);
					_stage.propertyPanel().codeEditor().set(data.properties);
				},
		...
		$('#propertyPanel_nodePath').val(nodePath);
