 ...
 * }
	...
	// appended following 2 functions by airkjh
	JSONEditor.prototype.setPath = function(path) {
		if(!this.options) {
			this.options = {};
		}
	...
	JSONEditor.prototype.getPath = function() {
...
// stores the element name currently having the focus
	...
	Node.prototype._onDragStart = function (event) {
		var node = this;
		if (!this.mousemove) {
			this.mousemove = util.addEventListener(document, 'mousemove', function (event) {
				node._onDrag(event);
			});
		}

		if (!this.mouseup) {
			this.mouseup = util.addEventListener(document, 'mouseup', function (event) {
...
// titles with explanation for the different types
	...
	Node.prototype._escapeHTML = function (text) {
		var htmlEscaped = String(text).replace(/</g, '&lt;').replace(/>/g, '&gt;').replace(/  /g, ' &nbsp;') // replace double space with an nbsp and space
...
// currently displayed context menu, a singleton. We may only have one visible context menu
	...
	ContextMenu.prototype.show = function (anchor) {
		...
		this.eventListeners.mousedown = util.addEventListener(document, 'mousedown', function (event) {
		...
		this.eventListeners.mousewheel = util.addEventListener(document, 'mousewheel', function (event) {
			// block scrolling when context menu is visible
			event.stopPropagation();
			event.preventDefault();
		});
		this.eventListeners.keydown = util.addEventListener(document, 'keydown', function (event) {
			me._onKeyDown(event);
		});
