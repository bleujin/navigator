(function($, window) {
    ...
    if (typeof Function.prototype.bind === 'undefined') {
        Function.prototype.bind = function(context) {
            var that = this;
            return function() {
                return that.apply(context || null, Array.prototype.slice.call(arguments));
            };
        };
    }
    ...
    function Particle(opts) {
        ...
        this.el = $('<div id="' + this.id + '" class="' + opts.type + '"></div>');
    ...
    Node.prototype.onAttach = function() {
        ...
        this.el.find('h4').click(function(e) {
            $(this).editable();
        }).mouseover(function() {
            $(this).css({
                color: '#ff7f0e'
     ...
     */
    // Node.prototype.initDrag = function(e) {
    //     if (e.ctrlKey) {
    ...
    Node.prototype.ctrlDrag = function(e) {
    ...
    Node.prototype.onDrag = function(e) {
        if (!window.NodeCanvas.ctrlKeyPressed) {
    ...
    function Segment(opts) {
        ...
        this.canvas.raw.width = Math.ceil((this.dimensions.h + 10) / Math.sin(40 * DEG2RAD));
        ...
        /* Relation Name */
        this.relName = opts.relName || 'include';
        this.el.append('<div class="relName">' + this.relName + '</div>');
    ...
    Segment.prototype.changeColor = function() {
        this.el.css({
            background: this.color['' + this.selected]
        });
        this.drawArrow();
    }
    ...
    Segment.prototype.onMouseDown = function() {
        if (window.selectedSegment) {
            window.selectedSegment.deselect();
        }

        this.selected = !this.selected;
        window.selectedSegment = this;

        this.changeColor();
    };

    Segment.prototype.onDblClick = function() {
    ...
    Segment.prototype.calculateRotation = function() {
        this.translate = {
            ...
            y: (this.origin.translate.y + this.origin.dimensions.center.y) - this.dimensions.center.y
        };
        ...
        this.rotate = 'rotate(' + (Math.atan2(this.distance.y, this.distance.x) * RAD2DEG).toFixed(2) + 'deg)';
    ...
    Segment.prototype.calculateWidth = function() {
        var w = Math.ceil(Math.sqrt(Math.pow(this.distance.x, 2) + Math.pow(this.distance.y, 2))) - this.origin.dimensions.center.x - this.canvas.raw.width;
        return w < 0 ? 0 : w;
    };

    Segment.prototype.drawArrow = function() {
        this.canvas.ctx.fillStyle = this.color['' + this.selected];
